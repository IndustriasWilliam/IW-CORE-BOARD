...
int32_t BSP_TS_Init(uint32_t Instance, TS_Init_t *TS_Init) {
	...
	if ((Instance >= TS_INSTANCES_NBR) || (TS_Init->Width == 0U)
		...
		if (ret == BSP_ERROR_NONE) {
			...
			if (Ts_Drv->GetCapabilities(Ts_CompObj[Instance], &Capabilities)
					< 0) {
				...
				for (i = 0; i < TS_TOUCH_NBR; i++) {
					Ts_Ctx[Instance].PreviousX[i] = TS_Init->Width
							+ TS_Init->Accuracy + 1U;
					Ts_Ctx[Instance].PreviousY[i] = TS_Init->Height
							+ TS_Init->Accuracy + 1U;
				}
			}
		}
...
int32_t BSP_TS_EnableIT(uint32_t Instance) {
	...
	if (Instance >= TS_INSTANCES_NBR) {
		...
		if (Ts_Drv->EnableIT(Ts_CompObj[Instance]) < 0) {
			...
			if (HAL_EXTI_GetHandle(&hts_exti[Instance], TS_EXTI_LINE)
					!= HAL_OK) {
				ret = BSP_ERROR_PERIPH_FAILURE;
			}
			else if (HAL_EXTI_RegisterCallback(&hts_exti[Instance],
					HAL_EXTI_COMMON_CB_ID, TS_EXTI_Callback) != HAL_OK) {
				ret = BSP_ERROR_PERIPH_FAILURE;
			}
			else {
				/* Enable and set the TS_INT EXTI Interrupt to an intermediate priority */
				HAL_NVIC_SetPriority((IRQn_Type) (TS_INT_EXTI_IRQn),
						5, 0x00);
 ...
 * @retval None.
 */
__attribute__((weak)) void BSP_TS_Callback(uint32_t Instance) {
...
int32_t BSP_TS_GetState(uint32_t Instance, TS_State_t *TS_State) {
	...
	if (Instance >= TS_INSTANCES_NBR) {
		...
		if (Ts_Drv->GetState(Ts_CompObj[Instance], &state) < 0) {
			...
			/* Apply boundary */
			TS_State->TouchX = (x_oriented * Ts_Ctx[Instance].Width)
					/ Ts_Ctx[Instance].MaxX;
			TS_State->TouchY = (y_oriented * Ts_Ctx[Instance].Height)
					/ Ts_Ctx[Instance].MaxY;
 ...
 * @retval BSP status
 */
int32_t BSP_TS_Get_MultiTouchState(uint32_t Instance,
		...
		if (Ts_Drv->GetMultiTouchState(Ts_CompObj[Instance], &state) < 0) {
			...
			if (state.TouchDetected != 0U) {
				for (index = 0; index < state.TouchDetected; index++) {
					...
					if ((Ts_Ctx[Instance].Orientation & TS_SWAP_X) == TS_SWAP_X) {
						x_oriented = Ts_Ctx[Instance].MaxX - state.TouchX[index]
								- 1UL;
					}

					if ((Ts_Ctx[Instance].Orientation & TS_SWAP_Y) == TS_SWAP_Y) {
						y_oriented = Ts_Ctx[Instance].MaxY
								- state.TouchY[index];
					}

					/* Apply boundary */
					TS_State->TouchX[index] = (x_oriented
							* Ts_Ctx[Instance].Width) / Ts_Ctx[Instance].MaxX;
					TS_State->TouchY[index] = (y_oriented
							* Ts_Ctx[Instance].Height) / Ts_Ctx[Instance].MaxY;
					/* Store Current TS state */
					TS_State->TouchDetected = state.TouchDetected;

					/* Check accuracy */
					x_diff =
							(TS_State->TouchX[index]
									> Ts_Ctx[Instance].PreviousX[index]) ?
									(TS_State->TouchX[index]
											- Ts_Ctx[Instance].PreviousX[index]) :
									(Ts_Ctx[Instance].PreviousX[index]
											- TS_State->TouchX[index]);
					...
					y_diff =
							(TS_State->TouchY[index]
									> Ts_Ctx[Instance].PreviousY[index]) ?
									(TS_State->TouchY[index]
											- Ts_Ctx[Instance].PreviousY[index]) :
									(Ts_Ctx[Instance].PreviousY[index]
											- TS_State->TouchY[index]);
 ...
 * @retval BSP status
 */
int32_t BSP_TS_GestureConfig(uint32_t Instance,
...
int32_t GT911_Probe(uint32_t Instance) {
	...
	if (GT911_RegisterBusIO(&GT911Obj, &IOCtx) != GT911_OK) {
		printf("BSP_ERROR_BUS_FAILURE !!! \r\n");
		ret = BSP_ERROR_BUS_FAILURE;
	}
	else if (GT911_ReadID(&GT911Obj, &gt911_id) != GT911_OK) {
		printf("BSP_ERROR_COMPONENT_FAILURE !!! %lu\r\n", gt911_id);
		ret = BSP_ERROR_COMPONENT_FAILURE;
	}
	else if (gt911_id != GT911_ID) {
		printf("BSP_ERROR_UNKNOWN_COMPONENT !!! %lu \r\n", gt911_id);
		ret = BSP_ERROR_UNKNOWN_COMPONENT;
	}
	else {
		printf("GT911 initialized success !!! \r\n");
